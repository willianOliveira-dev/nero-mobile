/**
 * Generated by orval v8.5.3 🍺
 * Do not edit manually.
 * Nero API
 * A Nero API é uma interface RESTful robusta projetada especificamente para o varejo de moda online. Ela oferece controle total sobre o ciclo de vida do produto, desde a entrada de estoque em múltiplas variantes (SKUs) até o processamento final do checkout. Ideal para marketplaces, lojas boutique ou aplicativos móveis de moda.
 * OpenAPI spec version: 1.0.0
 */
import type { CreateHomeSectionBodyFilterJson } from './createHomeSectionBodyFilterJson';
import type { CreateHomeSectionBodyType } from './createHomeSectionBodyType';

export type CreateHomeSectionBody = {
  /**
   * @minLength 2
   * @maxLength 80
   * @pattern ^[a-z0-9]+(?:-[a-z0-9]+)*$
   */
  slug: string;
  /**
   * @minLength 2
   * @maxLength 120
   */
  title: string;
  type: CreateHomeSectionBodyType;
  /**
   * @minimum 0
   * @maximum 9007199254740991
   */
  sortOrder?: number;
  isActive?: boolean;
  filterJson?: CreateHomeSectionBodyFilterJson;
};
